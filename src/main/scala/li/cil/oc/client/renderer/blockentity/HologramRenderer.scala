...
import li.cil.oc.client.renderer.blockentity.HologramRenderer.hologram
...
object HologramRenderer extends Function[BlockEntityRenderDispatcher, HologramRenderer]
  ...
  @SubscribeEvent
  def onRenderWorldLast(e: RenderLevelLastEvent): Unit = {
...
class HologramRenderer(dispatch: BlockEntityRenderDispatcher) extends BlockEntityRenderer[Hologram] {
  ...
  override def getViewDistance = (hologram.scale / Settings.get.hologramMaxScaleByTier.max * Settings.get.hologramRenderDistance).asInstanceOf[Int]
}
