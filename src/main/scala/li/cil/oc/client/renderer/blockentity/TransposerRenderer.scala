...
class TransposerRenderer(dispatch: BlockEntityRenderDispatcher) extends BlockEntityRenderer[blockentity.Transposer] {
  override def render(transposer: blockentity.Transposer, dt: Float, stack: PoseStack, buffer: MultiBufferSource, light: Int, overlay: Int) {
