...
class PowerDistributorRenderer(dispatch: BlockEntityRenderDispatcher) extends BlockEntityRenderer[blockentity.PowerDistributor] {
  override def render(distributor: blockentity.PowerDistributor, dt: Float, stack: PoseStack, buffer: MultiBufferSource, light: Int, overlay: Int) {
