...
class RobotRenderer(dispatch: BlockEntityRenderDispatcher) extends BlockEntityRenderer[blockentity.RobotProxy] {
  ...
  def renderChassis(stack: PoseStack, buffer: MultiBufferSource, light: Int, robot: blockentity.Robot = null, offset: Double = 0, isRunningOverride: Boolean = false) {
  ...
  override def render(proxy: blockentity.RobotProxy, f: Float, matrix: PoseStack, buffer: MultiBufferSource, light: Int, overlay: Int) {
      ...
      StackOption(robot.getItem(0)) match {
        case SomeStack(stack) =>
          ...
          try {
            ...
            itemRenderer.renderStatic(Minecraft.getInstance.player, stack, TransformType.THIRD_PERSON_RIGHT_HAND, false, matrix, buffer, proxy.getLevel, light, overlay, 0)
          }
          catch {
            case e: Throwable =>
