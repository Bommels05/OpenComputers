...
object Network extends api.detail.NetworkAPI {
  ...
  override def joinOrCreateNetwork(blockEntity: BlockEntity): Unit = {
    if (blockEntity != null) {
      val world = blockEntity.getLevel
  ...
  def getNetworkNode(blockEntity: BlockEntity, side: Direction): Option[ImmutableNode] = {
    if (blockEntity != null) {
      ...
      if (blockEntity.getCapability(Capabilities.EnvironmentCapability, side).isPresent) {
        val host = blockEntity.getCapability(Capabilities.EnvironmentCapability, side).orElse(null)
  ...
  private def canConnectBasedOnColor(be1: BlockEntity, be2: BlockEntity) = {
    val (c1, c2) = (getConnectionColor(be1), getConnectionColor(be2))
