...
import net.minecraft.world.entity.{Entity, EntityDimensions, EquipmentSlot, LivingEntity, Pose, player}
...
object Player {
  ...
  private val playerMainHandler = ObfuscationReflectionHelper.findField(classOf[player.Player], "playerMainHandler")

  private val playerEquipmentHandler = ObfuscationReflectionHelper.findField(classOf[player.Player], "playerEquipmentHandler")

  private val playerJoinedHandler = ObfuscationReflectionHelper.findField(classOf[player.Player], "playerJoinedHandler")
...
class Player(val agent: internal.Agent) extends FakePlayer(agent.world.asInstanceOf[ServerLevel], Player.profileFor(agent)) {
  connection= new ServerGamePacketListenerImpl(server, FakeConnection, this)
  ...
  {
    ObfuscationReflectionHelper.setPrivateValue(classOf[player.Player], this, new AgentInventory(this, agent), "f_36093_")
    // because the inventory was just overwritten, the container is now detached
    ObfuscationReflectionHelper.setPrivateValue(classOf[player.Player], this, new InventoryMenu(getInventory, !level.isClientSide, this), "f_36095_")
  ...
  def activateBlockOrUseItem(pos: BlockPos, side: Direction, hitX: Float, hitY: Float, hitZ: Float, duration: Double): ActivationType.Value = {
    callUsingItemInSlot(agent.equipmentInventory, 0, stack => {
      ...
      val traceCtx = if (state.isAir) {
        BlockHitResult.miss(traceEndPos, side, pos)
      } else new BlockHitResult(traceEndPos, side, pos, false)
  ...
  override def canHarmPlayer(player: entity.player.Player): Boolean = Settings.get.canAttackPlayers
  ...
  override def remove(removalReason: Entity.RemovalReason): Unit = super.remove(removalReason)
