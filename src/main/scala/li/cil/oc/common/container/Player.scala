...
import net.minecraft.world.inventory.{AbstractContainerMenu, ClickType, ContainerListener, HopperMenu, MenuType, Slot}
...
abstract class Player(selfType: MenuType[_ <: Player], id: Int, val playerInventory: Inventory, val otherInventory: Container) extends AbstractContainerMenu(selfType, id) {
  ...
  override def stillValid(player: entity.player.Player) = otherInventory.stillValid(player)

  override def clicked(slot: Int, dragType: Int, clickType: ClickType, player: entity.player.Player): Unit = {
  ...
  override def quickMoveStack(player: entity.player.Player, index: Int): ItemStack = {
