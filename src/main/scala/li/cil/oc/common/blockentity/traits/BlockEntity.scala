...
trait BlockEntity extends net.minecraft.world.level.block.entity.BlockEntity {
  ...
  def updateEntity() {
    if (Settings.get.periodicallyForceLightUpdate && getLevel.getGameTime % 40 == 0 && getBlockState.getBlock.getLightEmission(getLevel.getBlockState(getBlockPos), getLevel, getBlockPos) > 0) {
  ...
  override def getUpdatePacket: ClientboundBlockEntityDataPacket = {
    ...
    val be = this.asInstanceOf[net.minecraft.world.level.block.entity.BlockEntity]
