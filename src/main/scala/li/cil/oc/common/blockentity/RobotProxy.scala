...
class RobotProxy(selfType: BlockEntityType[_ <: RobotProxy], val robot: Robot, pos: BlockPos, state: BlockState) extends BlockEntity(selfType, pos, state)
  with traits.Computer with traits.PowerInformation with traits.RotatableBlockEntity with WorldlyContainer with IFluidHandler with internal.Robot {

  def this(selfType: BlockEntityType[_ <: RobotProxy], pos: BlockPos, state: BlockState) = this(selfType, new Robot(pos, state), pos, state)
