...
class Hologram(selfType: BlockEntityType[_ <: Hologram], var tier: Int, pos: BlockPos, state: BlockState) extends BlockEntity(selfType, pos, state) with traits.Environment with SidedEnvironment with Analyzable with traits.RotatableBlockEntity with traits.Tickable with DeviceInfo {
  def this(selfType: BlockEntityType[_ <: Hologram], pos: BlockPos, state: BlockState) = this(selfType, 0, pos, state)
