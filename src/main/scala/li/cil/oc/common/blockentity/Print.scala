...
class Print(selfType: BlockEntityType[_ <: Print], val canToggle: Option[() => Boolean], val scheduleUpdate: Option[Int => Unit], val onStateChange: Option[() => Unit], pos: BlockPos, blockState: BlockState)
  extends BlockEntity(selfType, pos, blockState) with traits.BlockEntity with traits.RedstoneAware with traits.RotatableBlockEntity with IModelData {

  def this(selfType: BlockEntityType[_ <: Print], pos: BlockPos, state: BlockState) = this(selfType, None, None, None, pos, state)
  def this(selfType: BlockEntityType[_ <: Print], canToggle: () => Boolean, scheduleUpdate: Int => Unit, onStateChange: () => Unit, pos: BlockPos, state: BlockState) =
    this(selfType, Option(canToggle), Option(scheduleUpdate), Option(onStateChange), pos, state)
