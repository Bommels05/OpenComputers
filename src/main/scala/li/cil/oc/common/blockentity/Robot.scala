...
import net.minecraft.world.entity.{EquipmentSlot, player}
...
class Robot(pos: BlockPos, state: BlockState) extends BlockEntity(BlockEntityTypes.ROBOT, pos, state) with traits.Computer with traits.PowerInformation with traits.RotatableBlockEntity
  ...
  def setLevelAndPosition(level: Level, pos: BlockPos): Unit = {
    setLevel(level)
    worldPosition = pos
  }
  ...
  override def createMenu(id: Int, playerInventory: Inventory, player: entity.player.Player) =
