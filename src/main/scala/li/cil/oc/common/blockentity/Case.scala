...
class Case(selfType: BlockEntityType[_ <: Case], var tier: Int, pos: BlockPos, state: BlockState) extends BlockEntity(selfType, pos, state) with traits.PowerAcceptor with traits.Computer with traits.Colored with internal.Case with DeviceInfo with MenuProvider {
  def this(selfType: BlockEntityType[_ <: Case], pos: BlockPos, state: BlockState) = {
    this(selfType, 0, pos, state)
