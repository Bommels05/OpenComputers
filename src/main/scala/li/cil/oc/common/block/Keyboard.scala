...
import net.minecraft.world.level.{BlockGetter, Level, LevelAccessor, LevelReader}
...
class Keyboard(props: Properties) extends SimpleBlock(props, blockentity.BlockEntityTypes.KEYBOARD) {
  ...
  override def newBlockEntity(pos: BlockPos, state: BlockState) = new blockentity.Keyboard(blockentity.BlockEntityTypes.KEYBOARD, pos, state)
  ...
  override def localOnBlockActivated(world: Level, pos: BlockPos, player: Player, hand: InteractionHand, heldItem: ItemStack, side: Direction, hitX: Float, hitY: Float, hitZ: Float) =
    adjacencyInfo(world, pos) match {
      case Some((keyboard, screen: Screen, blockPos, facing)) => screen.rightClick(world, blockPos, player, hand, heldItem, facing, 0, 0, 0, force = true)
      case _ => false
    }
