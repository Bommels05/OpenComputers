...
import net.minecraft.nbt.{ByteArrayTag, ByteTag, CompoundTag, DoubleTag, FloatTag, IntArrayTag, IntTag, ListTag, LongTag, ShortTag, StringTag, Tag}
...
object ExtendedNBT {
  ...
  class ExtendedListNBT(val nbt: ListTag) {
    ...
    def foreach[Tag <: minecraft.nbt.Tag](f: Tag => Unit) {
      val iterable = nbt.copy(): ListTag
      while (iterable.size > 0) {
        f((iterable.remove(0): minecraft.nbt.Tag).asInstanceOf[Tag])
      }
    }

    def map[Tag <: minecraft.nbt.Tag, Value](f: Tag => Value): IndexedSeq[Value] = {
      val iterable = nbt.copy(): ListTag
      val buffer = mutable.ArrayBuffer.empty[Value]
      while (iterable.size > 0) {
        buffer += f((iterable.remove(0): minecraft.nbt.Tag).asInstanceOf[Tag])
      }
      buffer.toIndexedSeq
    }
